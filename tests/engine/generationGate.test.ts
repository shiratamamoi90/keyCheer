// engine/providers/gate: 生成開始前の前提チェック(テキスト・音声をまとめて見る)。
...
import { checkGenerationGate } from "../../src/engine/providers/gate.js";
import type { ProviderSelection } from "../../src/shared/types.js";
...
describe("generationGate / 外部選択時は同意ダイアログを経る", () => {
  ...
  it("does not look at the image provider (画像生成は本 change のスコープ外)", () => {
...
describe("generationGate / APIキー未設定で外部プロバイダーを選んだ場合 [異常系]", () => {
  it("blocks on the missing key before asking for consent", () => {
    ...
    // (engine/providers/consent.ts の優先順と揃える)。
    const result = checkGenerationGate({
      selection: { ...LOCAL, text: "openai" },
      consent: {},
      hasApiKey: noKeys,
