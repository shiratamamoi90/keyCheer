...
} from "../../src/engine/providers/consent.js";

// spec: changes/0003-external-api-providers/spec.md / specs/integrations.md
// 同意はプロバイダー単位で 1 回。ローカルは同意不要。engine は状態遷移の純粋関数のみ
//(ダイアログ表示・永続化は main/UI 側)。

describe("consent / 外部選択時は同意ダイアログを経る", () => {
  it("requires a consent dialog for an external provider without consent", () => {
...
describe("consent / 同意済みプロバイダーへの再送信(同意はプロバイダー単位で 1 回のみ)", () => {
  it("after granting, no dialog is needed and sending is allowed", () => {
