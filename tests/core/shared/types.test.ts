...
// 要件: docs/data-model.md「speedZone は 3 値のみ [境界]」「timeOfDay は 4 値のみ [境界]」
//        「プロバイダー識別子の定義域 [境界]」/ docs/integrations.md「プロバイダー識別子(閉じた union)」
// これらの配列は core(バケットキー生成・バリデーション)の単一参照点なので、
...
describe("shared/types / speedZone は 3 値のみ [境界]", () => {
  it("S0015_03 exposes exactly slow / normal / fast in a deterministic order", () => {
    expect([...SPEED_ZONES]).toEqual(["slow", "normal", "fast"]);
  });
});

describe("shared/types / timeOfDay は 4 値のみ [境界]", () => {
  it("S0015_04 exposes exactly morning / afternoon / evening / night in a deterministic order", () => {
...
describe("shared/types / プロバイダー識別子の定義域 [境界]", () => {
  it("S0015_05 closes the text/voice/image unions to the documented identifiers", () => {
