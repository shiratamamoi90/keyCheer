...
import {
  ...
  EVENING_START,
  NIGHT_START,
} from "../../src/core/timeOfDay.js";

// spec: 論点 0014
describe("timeOfDay", () => {
  describe("代表時刻", () => {
    it("S0014_01 朝の代表時刻: classifies hour 7 as morning", () => {
      expect(classifyTimeOfDay(7)).toBe("morning");
    });

    it("S0014_02 昼の代表時刻: classifies hour 13 as afternoon", () => {
      expect(classifyTimeOfDay(13)).toBe("afternoon");
    });

    it("S0014_03 夕方の代表時刻: classifies hour 18 as evening", () => {
      expect(classifyTimeOfDay(18)).toBe("evening");
    });

    it("S0014_04 夜の代表時刻: classifies hour 23 as night", () => {
      expect(classifyTimeOfDay(23)).toBe("night");
    });
  });

  describe("深夜は night [境界]", () => {
    it("S0014_05 classifies hour 0 as night", () => {
  ...
  describe("night → morning 境界 [境界]", () => {
    it("S0014_06 classifies hour 4 (= MORNING_START - 1) as night", () => {
      expect(classifyTimeOfDay(4)).toBe("night");
    });
  ...
  describe("morning → afternoon 境界 [境界]", () => {
    it("S0014_07 classifies hour 10 (= AFTERNOON_START - 1) as morning", () => {
  ...
  describe("afternoon → evening 境界 [境界]", () => {
    it("S0014_08 classifies hour 16 (= EVENING_START - 1) as afternoon", () => {
  ...
  describe("evening → night 境界 [境界]", () => {
    it("S0014_09 classifies hour 20 (= NIGHT_START - 1) as evening", () => {
  ...
  describe("Date からの薄いラッパ", () => {
    it("S0014_10 delegates to classifyTimeOfDay(date.getHours())", () => {
