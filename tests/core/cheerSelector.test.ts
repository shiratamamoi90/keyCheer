...
import { bucketKey, ALL_BUCKET_KEYS, type MessagePool } from "../../src/core/messagePool.js";
...
describe("cheerSelector", () => {
  ...
  describe("シナリオ: 同一文の連続再生を避ける(セッション内除外方式)", () => {
    it("S0013_07 excludes the last messageId for this bucket", () => {
      const pool = makePool(() => [
        { id: "a", text: "alpha" },
        { id: "b", text: "beta" },
      ]);
  ...
  describe("シナリオ: マイルストーン文中の動的数値補間", () => {
    it("S0013_08 replaces {milestone} in milestone-type messages", () => {
  ...
  describe("シナリオ: 対象バケットが空のときのフォールバック [異常系]", () => {
    it("S0013_09 falls back to another zone with the same (type, timeOfDay)", () => {
      const pool = makePool((k) =>
