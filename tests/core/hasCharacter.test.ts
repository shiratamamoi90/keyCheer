// core/hasCharacter: キャラクター「作成済み」判定の純粋関数。
// spec: 論点 0018
//       「キャラクター未作成なら起動時にメインウィンドウを表示する」
//       「hasCharacter は必須フィールドの充足で判定する [境界]」

import { describe, it, expect } from "vitest";
import { hasCharacter } from "../../src/core/hasCharacter.js";
import type { Character } from "../../src/core/shared/types.js";
...
describe("core/hasCharacter / hasCharacter は必須フィールドの充足で判定する [境界]", () => {
...
// 画像生成は別の論点 のため、画像が無くても作成完了できる必要がある。
// これを許さないと画像を作るまで永久に「未作成」となり、起動のたびに
// メインウィンドウが自動表示されてしまう(0008 の自動表示ロジックへの波及)。
describe("core/hasCharacter / プロフィールが揃っていれば作成済みとする", () => {
...
describe("core/hasCharacter / 画像が無くても作成済みと判定する [境界]", () => {
...
describe("core/hasCharacter / プロフィールが欠けていれば未作成とする [境界]", () => {
