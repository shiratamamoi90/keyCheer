...
// 要件: docs/data-model.md「音声ファイル」/ docs/cheer-trigger.md
//       (プールからの 1 文選択 + 対応 wav 再生 / 音声ファイル欠損時のフォールバック / キャラ未作成時の発動)

import type { CheerSelection } from "../core/cheerSelector.js";

// messageId → wav パスの決定的解決(docs/data-model.md)。
...
// core の `CheerSelection` も main の `CheerEvent` も構造的にこれを満たす。
