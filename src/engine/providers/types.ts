...
// spec: changes/0003-external-api-providers/spec.md(生成手段の抽象化)/ specs/integrations.md
// 実装(Ollama/VOICEVOX/sd.cpp/外部 API)は src/agent 側。engine には型契約だけを置く。
...
export interface ImageGenerationRequest {
  prompt: string;
  seed: number; // 同一シード + 同一プロンプト = 同一画像(specs/integrations.md)
