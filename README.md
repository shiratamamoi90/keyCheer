...
- **renderer / preload**: 応援ポップアップと最小 preload API まで実装済み。メインウィンドウに**キャラ作成フォーム**(名前・性格・話者の入力と検証、VOICEVOX からの話者一覧取得、プロフィール保存)を実装済み。`npm run build` → `npm run dev` で**起動して打鍵に反応する**。
- **未実装**: 設定・統計 UI、**プール・wav の生成と読み書き**(キャラを保存しても生成は走らない)、同意ダイアログ、キャラ画像生成、配布インストーラ(electron-builder)。
