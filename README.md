...
- **engine(決定的コア)**: TDD 実装済み(カウント / アクティブ時間 / KPM / 速度ゾーン / トリガー / timeOfDay / プール選択 / baseline 定型文 / プロバイダー型契約・同意状態)。
- **agent(生成 I/O)**: プロバイダー実装済み(Ollama / VOICEVOX / sd.cpp / OpenAI text・TTS・DALL·E)、プール一括生成・全文事前合成・監査ログ・API キー(safeStorage)・再生計画。
- **main(Electron)**: 発動経路の配線済み(キーフック → engine → ポップアップ IPC・履歴記録、設定の読み込み/検証/移行、統計の定期 flush、トレイ、ウィンドウ)。
- **renderer / preload**: 応援ポップアップと最小 preload API まで実装済み。`npm run build` → `npm run dev` で**起動して打鍵に反応する**。
- **未実装**: 設定・統計 UI、キャラ作成フロー本体(プール・wav の読み書き、同意ダイアログ、コスト表示)、配布インストーラ(electron-builder)。
