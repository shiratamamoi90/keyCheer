...
`electron-store`(JSON)で永続化するデータ、メッセージプール、音声ファイル、API キー(OS セキュアストレージ)の構造を契約として固定する。型は `src/shared` に置き main/renderer で共有する。
...
- 同梱位置: `src/engine/baseline/messages.ts`(コード同梱、外部リソースファイルではない)
...
## シナリオ: `triggers.regular = 100` のマイグレーション [境界]
...
- baseline 定型文は `src/engine/baseline/messages.ts` にコード同梱。
