...
キャラ作成時に **(zone × type × timeOfDay) = 3 × 2 × 4 = 24 シナリオ × 約 20 文 ≒ 480 文**を一括生成し、メッセージプールとして保存する。
...
これから示すシナリオ {zone, type, timeOfDay} に合わせて 20 個生成してください。
...
- WHEN プール生成を要求
- THEN 24 シナリオ × 20 文 = 480 文がプールとして保存される(各文 30 文字以内、シナリオキーで引ける構造)

### シナリオ: 生成途中の中断と再開

- GIVEN 480 文一括生成中にプロバイダー失敗・ユーザーキャンセル
- WHEN 状態を保存
- THEN 生成済みシナリオは `completion: "complete"` で保持、未生成は `"pending"`、失敗は `"failed"` で残る。再開時は未完了(`pending` / `failed`)のシナリオのみ再生成する

### シナリオ: 一括生成中の進捗 UX

- GIVEN プール 480 文の一括生成中
- WHEN 生成が進行
...
プール 480 文を全文事前合成して wav 保存。発動時はこの wav を再生するだけ。

### シナリオ: 全文事前合成

- GIVEN プロバイダー設定済み、プール 480 文が確定
- WHEN 音声合成を要求
