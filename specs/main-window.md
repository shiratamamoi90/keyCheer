...
「いつ表示/非表示になるか」というライフサイクルのみ。詳細は `changes/0008-main-window-character-creation/`。

`hasCharacter` の判定・`onboarding.skipMainWindowAutoShow` のデータ契約は `specs/data-model.md` 参照。
...
## シナリオ: メインウィンドウは発動経路と独立している [不変条件]

- GIVEN メインウィンドウの表示/非表示状態(任意)
- WHEN キー押下がトリガー条件を満たす
- THEN メインウィンドウの状態に関係なくポップアップが発動する(相互に依存しない)

## シナリオ: メインウィンドウの renderer は providers を import しない [不変条件]

- GIVEN メインウィンドウ用 preload / renderer のソース
- WHEN import を検査する
- THEN `src/agent/*`・`src/engine/providers/*` を import しない(ESLint no-restricted-imports で機械的に検査)
