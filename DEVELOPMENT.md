...
> **現状**:`tests/integration/` は空。発動経路の縦 1 本(キー押下 → 応援表示)は
> 実機確認に頼っている。総合テストは追加予定。
...
- `src/core/` … 決定的なコアロジック。副作用を持たせない。カウント / アクティブ判定 / KPM /
  速度ゾーン / トリガー / 時間帯分類 / プール選択 / baseline 定型文 / プロバイダーの型契約。
- `src/core/shared/` … 型・IPC チャンネル契約・開示情報。core も app も参照する純粋な定義。
- `src/agent/` … 生成 I/O。プロバイダー実装(Ollama / VOICEVOX / sd.cpp / 外部 API)、
  プール一括生成、事前合成、監査ログ、API キー。
- `src/main/` `src/preload/` `src/renderer/` … Electron。キーフック・IPC・ウィンドウ・UI。
- `src/eval/` … 応援メッセージの品質・レイテンシ計測(`npm run eval`)。
...
- テストは実装と同じ区分で `tests/core` `tests/agent` `tests/main` `tests/preload`
  `tests/renderer`(単体)と `tests/integration`(総合)。
...
規律の本体は次の場所にあり、**既定では git 追跡外**(個人開発を前提にしているため)。

| 置き場 | 中身 |
| --- | --- |
| `CLAUDE.md` | エージェントへのルールと制約 |
| `.claude/rules/` | 要件 / 決定 / テスト / 実装 / 探索 / セキュリティ / 設計 / 技術スタックの各ルール |
| `.claude/skills/` | `/new-topic` `/decide` `/new-work` `/run-loop` `/validate` |
| `.claude/settings.json` | hooks(編集時の整形・停止時のテスト)。実体は `scripts/hooks.sh` |
| `TEMPLATE.md` | フォーマット自体の説明とディレクトリ構造・現在の状態の正本 |

- clone しただけではこれらは手に入らない。**チームで共有するなら `.gitignore` の該当行を消す**。
- 上の表の内容をこのファイルに書き写さない。写すと片方が古いまま残る。
- **例外:** このファイルの「ディレクトリと依存方向」「変更の進め方」は、規律を持たない
  読み手にも通じるように**意図して写している**。**ルール側(`impl-rules.md`「不変条件」/
  `doc-rules.md`「確定の流れ」)を変えたら、ここも必ず直す。**
